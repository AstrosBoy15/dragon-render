...
glm::vec2 WindowManager::getMouseDelta() {
	glm::vec2 deltaPos = getMousePosition() - lastMousePos;
	return deltaPos;
}

void WindowManager::setCursorPosition(glm::vec2 position) {
	glfwSetCursorPos(window, position.x, position.y);
}

//glm::vec2 WindowManager::getMouseWorldPosition(glm::vec2 world_scale) {
...
//		-getWindowHeight() / camera->getZoom() * mousePos.y) / getContentScale() / world_scale + camera->getPosition());
//}
