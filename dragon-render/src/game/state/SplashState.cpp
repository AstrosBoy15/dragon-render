...
void SplashState::init() {
	...
	bar_pos = glm::vec3(-(win_mgr->getWindowWidth() / 2) + 32,
		-(win_mgr->getWindowHeight() / 2) + 32, 0);
...
void SplashState::render() {
	calculateLoadingBar();
	renderer->setRenderMode(RenderModes::GUI);
	renderer->render(ModelGeometries::cube, 0, bar_pos, glm::vec3(0.0f), glm::vec2(load_width, 10), glm::vec4(1, 1, 1, 1));
}

void SplashState::calculateLoadingBar() {
	glm::vec3 target = glm::vec3((load_percent * (win_mgr->getWindowWidth() - 64)) / 2
		- win_mgr->getWindowWidth() / 2 + 32, -(win_mgr->getWindowHeight() / 2) + 32, 0);

	bar_pos = dragon::lerp(bar_pos, target, 0.07f);
	load_width = bar_pos.x - (-(win_mgr->getWindowWidth() / 2.0) + 32);
